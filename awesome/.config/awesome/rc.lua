...
for i = 1, math.min(9, #tagnames) do
  global_keys = gears.table.join(global_keys,
  -- View tag
  awful.key({ modkey }, "#" .. i + 9,
      function ()
        local screen = awful.screen.focused()
        local tag = screen.tagnames[i]
        if tag then
          tag:view_only()
        end
      end,
      { description = "View tag '" .. tagnames[i] .. "'", group = "awesome: tags" }),
      -- Add client to tag
      awful.key({ modkey, "Control" }, "#" .. i + 9,
      function ()
      if client.focus then
        local tag = client.focus.screen.tagnames[i]
        if tag then
          client.focus:toggle_tag(tag)
        end
      end
    end,
    { description = "Add tag '" .. tagnames[i] .. "' to focused client", group = "awesome: tags" }),
    -- Move client to tag
    ...
    { description = "Move focused client to tag '" .. tagnames[i] .. "'", group = "awesome: tags" })
    )
end
...
awful.rules.rules = {
  ...
  {
    rule_any = {
      instance = { "pavucontrol", "arandr", "pcmanfm", "blueman-manager", "lxappearance", "nm-connection-editor" },
      role = { "pop-up" }
    },
    properties = {
      floating = true,
